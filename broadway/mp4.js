...
var MP4Player = (function reader() {
  ...
  function constructor(data, useWorkers, opd) {
    // this.stream = stream;
    this.stream = data;
    ...
    this.avc = new Player({
      ...
      reuseMemory: false,
      transferMemory: false,
    });

    this.avc.onPictureDecoded = opd;
  ...
  constructor.prototype = {
    ...
    readDifferent: function(callback) {
      this.reader = new MP4Reader(new Bytestream(this.data));
      this.reader.read();
      var video = this.reader.tracks[1];
      this.size = new Size(video.trak.tkhd.width, video.trak.tkhd.height);
      console.info("MP4Player::readAll(), length: " +  this.reader.stream.length);
    ...
    play: function() {
      ...
      if (!reader) {
        ...
        // this.readDifferent(this.play.bind(this));
        return;
      };
      ...
      var avc = this.avc;
      for (var i = 0; i < 256; i++) {
        video.getSampleNALUnits(i).forEach(function (nal) {
          avc.decode(nal);
        });
      }
    }
